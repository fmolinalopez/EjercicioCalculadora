package org.molina.calculadoracorregida;
...
public class CalcActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private double lastPressedNumber = 0;
    ...
    private void makeOperation() {
        ...
        if (contador == 1){
            mAccumulator = lastPressedNumber;
        }
        contador++;
        double total = 0;
        opPressed = true;
    ...
    private void readNumber(Button button) {
        ...
        lastPressedNumber = Double.valueOf(button.getText().toString());
        String actualNumber;

        Log.d("CalcActivity", "mAccumulator = " + mAccumulator + " | mOp = " + mOp);

        if (!opPressed){
            actualNumber = resultTextView.getText().toString();
            if (actualNumber.equals("0")) {
